...
void Usage(void) {
    ...
    printf("     -i             input file name (av1c elementary stream)\n\n");
    printf("   Example:  hello-decode -sw  -i in.av1\n");
    printf("   To view:  ffplay -f rawvideo -pixel_format yuv420p -video_size "
           "[width]x[height] %s\n\n",
           OUTPUT_FILE);
    printf(" * Decode av1c elementary stream to raw frames in %s\n\n", OUTPUT_FILE);
...
int main(int argc, char *argv[]) {
    ...
    bitstream.CodecId = MFX_CODEC_AV1;
    ...
    // sts                      = MFXVideoDECODE_Query(session, &decodeParams, &decodeParams);
    // VERIFY(MFX_ERR_UNSUPPORTED == sts, "not unsupported\n");
    // if(decodeParams.mfx.FrameInfo.FourCC == NULL)
    //     printf("is null");
    // else 
    //     printf("not null");
    ...
    VERIFY(MFX_ERR_INVALID_VIDEO_PARAM == sts, "not params error\n");
